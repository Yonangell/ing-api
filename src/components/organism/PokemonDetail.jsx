...
export default function PokemonDetail({ pokemon }) {
  // Formateamos las unidades según la documentación de PokeAPI
  ...
  return (
    <div className="max-w-md mx-auto p-6 bg-white dark:bg-gray-800 rounded-2xl shadow-xl border border-gray-100 dark:border-gray-700 transition-all hover:shadow-2xl">
      ...
      <div className="flex justify-between items-center mb-4">
        <span className="text-sm font-mono font-bold text-gray-400">#{pokemon.id.toString().padStart(3, '0')}</span>
        <PokemonTypes types={pokemon.types} />
      </div>

      {/* Imagen con un fondo sutil */}
      ...
      <div className="text-center space-y-4">
        ...
        {/* Sección de Stats (Si vienen en el objeto pokemon) */}
        <div className="space-y-3">
          <h3 className="text-left text-sm font-bold text-gray-600 dark:text-gray-300">Estadísticas Base</h3>
          <div className="grid grid-cols-1 gap-2">
            {pokemon.stats?.map((s) => (
              <div key={s.stat.name} className="flex items-center">
                <span className="w-24 text-left text-xs capitalize text-gray-500">{s.stat.name.replace('-', ' ')}</span>
                ...
                <span className="w-8 text-right text-xs font-bold ml-2">{s.base_stat}</span>
              </div>
            ))}
          </div>
        </div>
